...
class CDCL {
    ...
    val minimizeMarks = MutableList(1000_000) { 0 }
    var currentMinimizationMark = 1
     ...
     */
    private fun analyzeConflict(conflict: Clause): Clause {        var activeVariableCount = 0
            ...
            learnt = Clause(
                learntLits.filter { possiblyImpliedLit ->
                    assignment.reason(possiblyImpliedLit.variable)?.lits?.any {
                        it != possiblyImpliedLit.neg && minimizeMarks[it] != currentMinimizationMark
                    } ?: true
                }.toMutableList(),
                learnt = true,
            )
